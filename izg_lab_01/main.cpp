...
void printHelp()
{
    ...
    printElement("1", " - very low threshold", 15);
    printElement("2", " - low threshold", 15);
    printElement("3", " - medium threshold", 15);
    printElement("4", " - high threshold", 15);
...
void onKeyboard(SDL_KeyboardEvent* key, ImageTransform* transform)
{
    switch (key->keysym.sym)
    {
        ...
        case SDLK_g:
            ...
            break;
        case SDLK_1:
            transform->cfg->threshold = 50;
            IZG_INFO("Thresholding with t = ", std::to_string(transform->cfg->threshold));
            transform->threshold();
            break;
        case SDLK_2:
            transform->cfg->threshold = 100;
            IZG_INFO("Thresholding with t = ", std::to_string(transform->cfg->threshold));
            transform->threshold();
            break;
        case SDLK_3:
            transform->cfg->threshold = 150;
            IZG_INFO("Thresholding with t = ", std::to_string(transform->cfg->threshold));
            transform->threshold();
            break;
        case SDLK_4:
            transform->cfg->threshold = 200;
            IZG_INFO("Thresholding with t = ", std::to_string(transform->cfg->threshold));
            transform->threshold();
            break;
        case SDLK_t:
            // reset threshold
            transform->cfg->threshold = 127;
            IZG_INFO("Thresholding with t = ", std::to_string(transform->cfg->threshold));
            transform->threshold();
            break;
        case SDLK_r:
...
int main()
{
    Config cfg =  { 800, 600, true, 127};

    auto window   = SDL_CreateWindow("IZG lab 01", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, cfg.width, cfg.height, 0);
    ...
    while (cfg.running)
    {
        ...
        if (cfg.width != (uint32_t)surface->w || cfg.height != (uint32_t)surface->h)
        {
